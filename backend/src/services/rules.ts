...
export async function checkBorrowLimit(
  ...
  if (!patronCategoryId) {
    if (currentCount >= DEFAULTS.maxBorrows) {
      return { allowed: false, currentCount, maxBorrows: DEFAULTS.maxBorrows, message: `已达到借阅上限 ${DEFAULTS.maxBorrows} 册` }
    }
  ...
  if (currentCount >= rule.maxBorrows) {
    return { allowed: false, currentCount, maxBorrows: rule.maxBorrows, message: `已达到借阅上限 ${rule.maxBorrows} 册` }
  }
