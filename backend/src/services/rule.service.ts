...
import type { CirculationRuleResponse, RuleUpsertParams, PatronCategoryResponse, ItemTypeResponse } from '../types/api.types.js'
...
export async function listPatronCategories(prisma: PrismaClient): Promise<PatronCategoryResponse[]> {
...
export async function upsertRule(prisma: PrismaClient, data: RuleUpsertParams): Promise<CirculationRuleResponse> {
  return prisma.circulationRule.upsert({
    where: { patronCategoryId_itemTypeId: { patronCategoryId: data.patronCategoryId, itemTypeId: data.itemTypeId } },
    update: data,
    create: data
  }) as unknown as CirculationRuleResponse
}
