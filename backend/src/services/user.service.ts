...
export async function getReaderDetail(prisma: PrismaClient, id: number): Promise<ReaderDetail | null> {
  return prisma.user.findFirst({
    ...
    select: {
      ...
      borrowRecords: {
        ...
        orderBy: { borrowDate: 'desc' }, take: 50
      }
    }
  }) as unknown as ReaderDetail | null
}

export async function updateReader(prisma: PrismaClient, id: number, data: { name?: string; phone?: string; email?: string }) {
...
export async function updateProfile(prisma: PrismaClient, userId: number, data: { name?: string; phone?: string; email?: string }) {
  return prisma.user.update({
    where: { id: userId },
    data,
    select: { id: true, username: true, name: true, role: true, phone: true, email: true }
