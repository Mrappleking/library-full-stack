...
describe('register', () => {
  it('1. 正常注册 — 返回 user+token', async () => {
    ...
    const result = await authService.register(prisma, jwt, { username: 'test', password: 'pw', name: 'Test' })
  ...
  it('2. 用户名重复 — 409', async () => {
    ...
    await expect(authService.register(prisma, jwt, { username: 'test', password: 'pw', name: 'Test' }))
...
describe('login', () => {
  it('3. 正确密码 — 200', async () => {
    const prisma = mockPrisma()
    prisma.user.findUnique.mockResolvedValue({ id: 1, username: 'test', password: 'hash', name: 'T', role: 'reader', createdAt: new Date() })
  ...
  it('4. 错误密码 — 401', async () => {
    ...
    await expect(authService.login(prisma, jwt, 'test', 'wrong'))
...
describe('getMe', () => {
  it('5. 有效用户 — 返回信息', async () => {
    const prisma = mockPrisma()
    prisma.user.findUnique.mockResolvedValue({ id: 1, username: 'test', name: 'T', role: 'reader', createdAt: new Date().toISOString() })
  ...
  it('6. 不存在 — 404', async () => {
    ...
    await expect(authService.getMe(prisma, 999))
