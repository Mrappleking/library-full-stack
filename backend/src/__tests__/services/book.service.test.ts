...
function mockPrisma(overrides: Record<string, any> = {}) {
  return {
    book: { findMany: vi.fn(), findUnique: vi.fn(), count: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn(), groupBy: vi.fn() },
...
describe('create', () => {
  it('4. total > 0 — 自动设 available = total', async () => {
    ...
    const result = await bookService.create(prisma, {
      isbn: 'X', title: 'New', author: 'A', total: 3, categoryId: 1
    })
...
describe('update', () => {
  ...
  it('6. total 缩小到借出量以下 — 拒绝', async () => {
    ...
    await expect(bookService.update(prisma, 1, { total: 2 }))
      .rejects.toThrow('Cannot reduce total')
  })
})
